...
function initializeDB (path, cb) {
  store = {}
  filePath = path
}

function loadDB (path) {
  fs.readFile(path, 'utf8', 'r+', (err, data) => {
    if (err) throw err
    if (data === undefined) store = {}
    else store = JSON.parse(data)
  })
}

function commit () {
  fs.writeFile(filePath, JSON.stringify(store, null, 4), 'utf8', 'w+', (err) => {
    if (err) throw err
  })
  return true
}
...
function nestedRemove (keyArr) {
  ...
  for (i = 1; i < keyArr.length - 1; i++) {
    if (keyArr[i] in db) {
      if (typeof db[keyArr[i]] === 'object') db = db[keyArr[i]]
      else throw new Error('Invalid deletion')
    } else throw new Error('Invalid key')
  }
